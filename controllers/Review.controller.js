...
exports.UpdateReview = function (req, res) {
	...
	var ReviewId = (req.params.ReviewId) ? req.params.ReviewId : false;
	if (ReviewId) {
		ReviewServices.UpdateReview(ReviewId, req.body).then(function (result) {
...
exports.DeleteReview = function (req, res) {
	...
	var ReviewId = (req.params.ReviewId) ? req.params.ReviewId : false;
	if (ReviewId) {
		ReviewServices.DeleteReview(ReviewId).then(function (result) {
			if (result)
				res.json({ "StatusCode": 200, "ResponseMessage": "Deleted Review Successfully" });
			else
				res.json({ "Message": "An error has occurred." });
		}).catch(function (err) {
			res.json({ "StatusCode": err.status, "deleteReview": [], "ResponseMessage": err.messages });
		});
