var config = require('../config'),
	...
	SubCategories = require('../models/SubCategory.model'),
...
exports.GetCategoryWithSubCategory = function (req, res) {
	/* Method to get all the categories along with subcategories */
	CategoriesServices.getAllCategoriesWithSubCategories({ withRelated: ['SubCategory'] }).then(function (result) {
		if (result) {
			return Promise.map(result.models, function (category) {				
				return SubCategories.forge().query(function (qb) {
					qb.where('CatId', category.get('CatId'))
				}).fetchAll().then(function (subCat) {
					category.set("SubCategories", subCat);
					return category;
				}).catch(function (err) {
