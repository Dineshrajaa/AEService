	...
	password = helperServices.encryption(req.body.Password,req.body.EmailId);
		...
		if(result==0){
			...
			return UserAccountServices.registerUser(req.body).then(function(result1){
				res.json({"StatusCode":200,"user":result1,"ResponseMessage":"New user created successfully!"});
			}).catch(function(err){
				res.json({"StatusCode":err.status,"user":[],"ResponseMessage":err.messages});
			});
			
		}else{
			res.json({"StatusCode":302,"user":[],"ResponseMessage":"User Alredy Exist!!"});
		}
	...
	if(UserId){
			UserAccountServices.GetUserAccount(UserId).then(function(result){
				...
				if(!result){
					...
					res.json({"StatusCode": 200,"userAccount":fresult,"ResponseMessage": "Wow!! you are so passionate about your UserAccountRepository"});
			}).catch(function(err){
					res.json({"StatusCode":err.status,"Favourites":[],"ResponseMessage":err.messages});
			});
	}else{
		res.json({"Message": "No HTTP resource was found that matches the request URI '"+config.webUri+"/Aesthetic/api/Users/GetById'."});
	}
...
exports.GetUsersImageById = function(req, res){
	...
	if(UserId){
		UserAccountServices.GetUsersImageById(UserId).then(function(result){
			...
			console.log(fresult);
			if(fresult.length){
				res.json({"StatusCode": 200,"lstUser":fresult,"ResponseMessage": "Wow!! you are so passionate about your UserImageRepository"});
			}else{
				res.json({"Message": "An error has occurred."});
			}

		}).catch(function(err){
			res.json({"StatusCode":err.status,"lstUser":[],"ResponseMessage":err.messages});
		});
	}else{
		res.json({"Message": "No HTTP resource was found that matches the request URI '"+config.webUri+"/Aesthetic/api/Users/GetUsersImageById'."});
	}
}
...
return UserAccountServices.GetUserAccount(data1.UserId).then(function(result){
		if(!result){
			...
			if(password == result.get('Password')){
				...
				res.json({"StatusCode": 401,"userAccount": {"UserId": 149,"UserImage": null,"FirstName": null,"LastName": null,"Password": null,"ConfirmPassword": null,"EmailId": null,"ConfirmEmail": null,"Gender": null,"Address": null,"Town": null, "City": null,"PostCode": 0,"Profile": null,"TagLine": null,"DOB": null,"Role": null,"IsActive": null,"CreateDate": null,"ModifyDate": null,"oldpassword": "12345688888","Newpassword": "1234567","Fullname": null},"ResponseMessage": "Invalid Password"});
			}
		}
	}).catch(function(err){
